...
static inline audio_type apf_fxd_1st(audio_type x, coef_type k, audio_type *st_y, audio_type *st_x){
    acum_type  acum = 0;
    acum = fxd_mac_m(acum, k, fxd_rshift(x, ROOM1));    //Q61 31' + 30 ()

    acum = fxd_msub_m(acum, k, *st_y);                  //Q61 31' + 30

    acum = fxd63_add(acum, fxd63_lshift(*st_x, COEF_FR));  // Q30->Q61

    *st_x = fxd_rshift(x, ROOM1);   //Q30


    *st_y = (audio_type)saturation(fxd63_rshift(acum, COEF_FR - 1)); //Q61' > 
